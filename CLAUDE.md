...
- **문제**: **같은 항이 문제지 자에는 없었다.** 그래서 해설 과소는 6.7% 인데
  문제지 과소는 17.9% 였고, 그 17.9% 의 **97.4%가 세로 수식이 있는 문항**이었다.
  세로 수식 0개면 평균 오차 +4.0px(안전), 6~10개면 −36.3px(놓침).
- **해결**: 상수·정규식을 그대로 문제지 자에도 걸었다. 놓침 105 → 21 · 99 → 18.
...
- **교훈**: 같은 규칙을 쓰는 자리가 둘이면 **한 숫자를 두 곳이 쓰게** 하고,
  ...
  «판정의 성적»으로 적지 말 것** — 그림 치수를 채운 문항에서 99.2%, 앞으로 들어올
  문항(적재가 안 채웠다)에서 60.4% 였다. 그 사실이 보고서에 없으면 숫자는 하루짜리다.
