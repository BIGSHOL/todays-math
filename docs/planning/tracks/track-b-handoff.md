...
정답 **726건을 6단계로 나눠 고쳤고**, 값 판단이 필요한 **432건과 빈 정답 86건은 손대지 않고**
원장님 확인 목록(`docs/planning/14-answer-conflicts-review.md`, **60건**)으로 넘겼다.
...
## 3. 안 한 것 — 왜 남아 있고, 다음 사람은 무엇을 근거로 판단하나

숫자는 **2026-08-16 마감 시점**이다. 트랙 F 가 시험지 300편을 새로 적재해
세션 중에도 늘었으니, 이어받을 때 먼저 다시 재라(§4 재생성 명령).

| 남은 것 | 건수 | 판정 |
|---|---|---|
| 값이다름 | **147** | 진짜오답 |
| 소문항불일치 | **95** | 진짜오답 |
| 우리일부만 | 1 | 진짜오답 (새로 들어온 몫) |
| 공식훼손 | 151 | 판정불가 |
| 공식일부만 · 번호↔값 | 19 · 19 | 판정불가 |
| 빈 정답 (공식엔 값이 있음) | **86** | — |

### 3.1 값이다름 147 · 소문항불일치 95 — 근거가 부족하다

**「진짜오답」은 *DB 와 공식이 다르다*는 뜻이지 *DB 가 틀렸다*는 뜻이 아니다.**

표본 26건을 원본 정답면을 렌더해 눈으로 대조한 결과가 판단의 근거다:
...
그 12건에서 규칙 6개를 새로 얻어(인수순서·겉괄호·부등호방향·등호표기·소문항번호없음·
조각포함) 「값이다름」을 232 → 147 로 줄였지만, **남은 것에도 표기 차이가 섞여 있다.**
값을 덮으면 맞는 답을 틀리게 바꾼다. 그래서 손대지 않았다.

**다음 사람이 쓸 근거는 셋이다:**

1. **지면을 오려 눈으로 본다** — 가장 확실하다. 토큰 0·API 0.
   `python scripts/qa/shot-official-answer.py <examId>-<번호> ...`
   `--from-report <갈래> --limit N` 으로 갈래에서 표본을 자동으로 뽑을 수도 있다.
2. **표본에서 규칙을 더 얻는다** — 지금까지 두 차례(각 12~14건) 돌려 규칙 6개를 얻었다.
   규칙을 넣을 때는 `answerNotation.test.ts` 의 「흡수 금지」 픽스처가 통과하는지 볼 것.
   그게 통과하는 한 규칙은 안전한 쪽으로만 넓어진다.
3. **양쪽 다 순수 숫자인 것만 추린다** — 표기로 설명할 여지가 없어 사람 판단이 꼭 필요한
   핵심이다. 지금 **29건**이고 문서 14 §1 에 있다.

⚠️ **AI 로 다시 풀지 마라.** 이미 6,000건 넘게 AI 로 풀었고 그 검증이 AI 끼리라
못 잡았던 것이 이번 작업의 출발점이다. 바깥 출처(인쇄된 정답면)가 정본이다.
...
| 갈래 | 처리 |
|---|---|
| 순수 수치·짧은 식 | 바로 채울 수 있다 |
| 근호 가로선 잔재 `√⁄5` | `√5` 로 되돌린다 (규칙 있음) |
| **위첨자 소실 `e15`(=e¹⁵) `a2+b2`** | **보류 — 그대로 인쇄하면 틀린 값이다** |
| `(서술형)` 처럼 값이 아닌 것 | 폐기 후보 |

`apply-official-answers.ts` 의 `printable()` 이 「글자 뒤에 숫자」 낌새를 보류로 잡아낸다.
**다음 사람이 쓸 근거**: 위첨자 소실은 정답면 글리프 좌표(위첨자는 베이스라인이 올라가 있다)로
복원할 수 있다. 그 전에 `--fill` 을 돌리면 훼손이 그대로 인쇄된다.

### 3.3 정답 없는 532건 — 원본에 답이 없다

이관 결함이 아니다. 정답 없는 기출 178건 중 **149건은 HWP 시험지도 뽑혔고 그 번호도
있는데 `answer` 필드가 전부 비어 있다.** RPM 280건도 원본에서 회수 불가.

→ 다만 **178건 중 148건에 HWP `solution`(해설)이 있다.** 서술형·증명의 「해설참조」를
대체할 후보다. 해설 전문을 `answer` 에 넣으면 학생 시험지에 그대로 인쇄되므로
**넣는 방식은 제품 결정**이다.

### 3.4 원장님이 눈으로 보실 것 — **문서 14 에 60건**

`docs/planning/14-answer-conflicts-review.md`

| 절 | 무엇 | 건수 |
|---|---|---|
| §1 | 기출 — 양쪽 다 순수 숫자인데 값이 다름 | **29** (그 밖 28 · 자릿수 포함 1) |
| §2 | RPM — 번호 충돌 | **3** |
| §3 | 한글이 섞인 수식 — 인쇄 표기 결정 | **10** |
| §4 | 훼손돼 답으로 안 읽히는 값 | **18** |
| | **합계** | **60** |

> §3·§4 가 처음 보고보다 늘었다. §3 은 `\text` 3건만이 아니라 **한글이 섞인 것 전부**를,
> §4 는 괄호가 안 맞는 2건만이 아니라 **`\square` 자리표가 든 17건 전부와
> 분수 뒤에 숫자가 붙은 1건**을 담았기 때문이다. 같은 성격이라 함께 두는 것이 맞다.

**D-50 (복수정답 `③, ④`) 은 확정됐지만 인쇄 항목이라 완료 조건이 실물 프린터
출력 검수다(CLAUDE.md 6번) — 「원장님 실물 검수 대기」.**
...
`npm run type-check` · `npm run lint`(기존 경고 1) · `npm test` **878건 통과**.
(병합으로 늘었다. 병합 뒤에는 워크트리마다 `npx prisma generate` 를 한 번 돌려야
tsc 가 통과한다 — 안 돌리면 남의 트랙 파일에서 오류가 나 내 것으로 오해하기 쉽다.)
