...
(30행) 그 스크립트도 자기 디렉터리를 `sys.path` 맨 앞에 넣어 `PYTHONPATH` 로도 못 비킨다.
...
- **뿌리에 둔다**: `scripts/qa/` 와 `scripts/figure/` 로 갈려 있었다. 기다리는 쪽이
  **손으로 적은 경로 목록**으로 훑는데, 그 목록은 샌다 — 새 하네스가 새 이름을 쓰면
  누가 27행에 적어 주기 전까지 안 보이고, **적어 줄 사람이 없으면 그 자리는 구조적으로 0**
  이 된다. 뿌리 glob 하나면 적을 목록이 아예 없어진다.
