...
| `cylinder` / `cone` / `sphere` | 원기둥·원뿔·구. 밑면 타원은 **축에 나란**(ry/rx≈0.30).<br>원뿔은 꼭대기→밑면 접선 + 보이는 호. 삼각형+타원 금지 |
...
- **`cylinder`·`cone`**: 밑면 타원은 **축에 나란**하고 ry/rx ≈ 0.30(교과서 0.25~0.35).
...
`elem_advanced.py`: `HIDDEN_DASH` `5 4` · `ROUND_RATIO` 0.30 · `NGON_MIN_GAP` 0.18.
...
→ 둥근 입체만 깊이축을 **화면 위(90°)** 로 세운다. 그러면 축에 나란한 타원이 되고
   `depth_ratio` 가 곧 ry/rx 다(0.30). 옆면은 위·아래 **앞쪽 호를 이어** 만들고,
