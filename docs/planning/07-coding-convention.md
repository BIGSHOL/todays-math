...
| D-54 | 확인테스트가 **단원을 고루 훑는다** | `pickTypeBalanced` 가 지면(D-52) 다음으로 **«단원 사용 수 + 유형 사용 수»가 가장 작은 후보**를 고른다. 합이 같으면 단원이 적은 쪽, 그것도 같으면 시드 셔플 순서. **우선순위를 매기지 않는 것이 요점이다** — 단원을 앞에 두면 유형 편중이, 유형을 앞에 두면 커버리지가 그만큼 나빠진다. 실측(30개 조합 × 시드 30개): 커버리지 66.2% → **95.9%**, 한 단원 최다몫 38.8% → **20.3%**, 유형 최다몫 33.8% → 37.3%, 「같은 유형 3연속」과 인쇄 경고는 **전후 모두 0%**. **일일테스트는 한 글자도 안 바뀐다**(단원이 하나라 합이 유형만으로 갈린다). 난이도 배분·`INSUFFICIENT_PROBLEMS` 도 안 건드린다 — 제외가 아니라 후순위다. 상세는 `tracks/reports/review-spread.md` | 2026-08-19 원장님 확정 |
