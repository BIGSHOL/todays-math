...
| D-55 | 그림에 기대는 문항은 **변형본 채택을 막는다** (임시) | 변형은 본문 글자만 오가고 **그림은 따라가지 않는다** — `transform/adopt` 는 `figureUrls`/`figureSvg` 를 만들지 않는다. 그래서 그림 문항을 변형하면 「본문은 그림을 가리키는데 그림이 없는」 문항이 새로 태어난다. 이 저장소가 856건 잠그며 정리한 바로 그 부류다(16-figure-recovery-ledger). **실측(2026-08-19 공유 DB): 출제 가능 46,681건 중 그림 있음 9,419(20.2%) · 그중 본문이 그림을 지목 5,929. 막지 않으면 전부 통로다.** 판정은 `src/lib/figure/transformFigureBlock.ts` 한 곳 — ㉠ 원본에 그림이 붙어 있다(9,419건 전부 이쪽) 또는 ㉡ 본문이 그림을 지목한다(`classifyFigureNeed`=유실, 현재 0건 — 이미 잠긴 856건이 빠져서다. 새로 들어올 데이터용 가드). **막는 값은 화면·서버 양쪽에 세운다** — 화면만 막으면 문지기가 브라우저에 있는 것이고 그건 없는 것과 같다. 비용: 변형 못 하는 단원 241개, **전량 막히는 단원 8개**(전체 443). ⚠️ **임시다.** 원장님 방침(2026-08-19): 「도형은 SVG 엔진(`F:\시험지변환기`, FigureSpec v2 = LLM 생성 경로)으로 새로 만들고, 스캔 그림은 쓸 수 있는 AI 를 나중에 정한다」. 도형 생성이 붙으면 이 판정은 「엔진이 못 그리는 것만」으로 좁아진다. 규칙 파일이 `scripts/qa/` 에서 `src/lib/figure/` 로 옮겨 온 이유가 이것 — 세는 쪽·잠그는 쪽·**막는 쪽**이 한 파일을 읽는다 | 2026-08-19 원장님 확정 |
