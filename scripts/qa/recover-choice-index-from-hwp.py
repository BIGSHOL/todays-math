...
def main() -> None:
    ...
    for eid in sorted(targets):
        ...
        if len(nums) != len(set(nums)):
            ...
            continue
        # ⚠️ **보기에 붙은 그림이 하나도 없으면 그건 회수가 아니다.**
        # 규약상 전부 0(발문 그림)은 «올바른 배열»이라 검산을 통과하고, 그대로 넣으면
        # 이 행이 **회수됨**으로 잠금까지 풀린다. 그런데 이 행의 결함은 그대로다 —
        # 실측 `5181-13` 은 선택지가 「ㄱ ㄴ ㄷ」 짝짓기 **글자**이고 그림 여섯은 전부
        # 발문·<보기> 쪽이다. 「어느 그래프가 ㄱ인가」는 이 컬럼이 담는 값이 아니다.
        # 못 푸는 문항이 «회수됨»으로 풀려 나가느니 «모른다»로 두는 게 낫다.
        if not nums:
            bump("보기에 붙은 그림이 없다 (①~⑤ 가 아니라 ㄱㄴㄷ 짝짓기다)")
            if a.list:
                print("  X " + eid + " 그림이 전부 발문 쪽 — 이 컬럼이 담을 짝이 아니다")
            continue
