/**
 * 적대적 재현 (읽기 전용, 삭제 가능) — "컷오프 이후 자료는 어떤 경로로도 근거가 될 수 없다"
 * 를 경로별로 찔러 본다.
 */
...
const sessionState = vi.hoisted(() => ({ user: null as { id: string; email: string; name: string } | null }));
vi.mock("@/lib/session", () => ({ getSessionUser: vi.fn(async () => sessionState.user) }));
...
describe("[적대적] 누출 경로 전수", () => {
  it("① 학기 경계 — 2025-2-기말 은 2026-1-중간 예측의 근거로 쓸 수 있어야 한다(막히면 과잉차단)", async () => {
    seedExam({ externalExamId: "가람-2025-2-기말", school: SCHOOL, year: 2025, semester: 2, round: "기말", questions: standardQuestions() });
    const res = await post({ series: SERIES, targetPeriod: { year: 2026, semester: 1, round: "중간" } });
  ...
  it("② 컷오프==대상 — 대상 회차 시험지 자체가 DB 에 있어도 근거로 안 잡힌다", async () => {
    seedExam({ externalExamId: "가람-2025-2-기말", school: SCHOOL, year: 2025, semester: 2, round: "기말", questions: standardQuestions() });
    seedExam({ externalExamId: "가람-2026-1-중간(정답지)", school: SCHOOL, year: 2026, semester: 1, round: "중간", questions: standardQuestions() });
    const res = await post({ series: SERIES, targetPeriod: { year: 2026, semester: 1, round: "중간" } });
    const run = (await res.json()).data;
    console.log("  ② inputExamIds =", JSON.stringify(run.inputExamIds));
  ...
  it("④ 코호트 경로 — 다른 학교의 대상시점 시험지도 안 잡힌다", async () => {
    seedExam({ externalExamId: "가람-2025-2-기말", school: SCHOOL, year: 2025, semester: 2, round: "기말", questions: standardQuestions() });
    seedExam({ externalExamId: "나래-2026-1-중간", school: "나래중", year: 2026, semester: 1, round: "중간", questions: standardQuestions({ count: 25 }) });
    const res = await post({ series: SERIES, targetPeriod: { year: 2026, semester: 1, round: "중간" } });
    const run = (await res.json()).data;
    console.log("  ④ inputExamIds =", JSON.stringify(run.inputExamIds));
