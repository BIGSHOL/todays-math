...
function runRow(predictorParams: Record<string, unknown>) {
  return {
    ...
    params: {
      ...
      evidence: {
        history: 1, cohort: 0, rangeHistory: 0, rangeCohort: 0,
        excludedByTrust: 0, pinned: false,
      },
...
describe("[적대적] 저장된 run 과 현재 파라미터 스키마의 표류", () => {
  it("현재 엔진이 저장한 run 은 당연히 읽힌다 (대조군)", () => {
    expect(() => serializePredictionRunDetail(runRow({ ...DEFAULT_PARAMS }))).not.toThrow();
  });
  ...
  it("엔진이 파라미터를 하나 늘리기 **전에** 저장된 run 도 계속 읽혀야 한다", () => {
    ...
    console.log("  결과:", err ? `throw → ${(err as Error).message}` : "정상 조회");
    expect(err).toBeNull();
  });

  it("엔진이 파라미터를 **줄인 뒤** 옛 run 을 읽어도 계속 읽혀야 한다", () => {
    const old = { ...DEFAULT_PARAMS, retiredKnob: 0.3 } as Record<string, unknown>;
    let err: unknown = null;
    try {
      serializePredictionRunDetail(runRow(old));
    } catch (e) {
