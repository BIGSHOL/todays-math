...
export const ITEMS: PrintCheckItem[] = [
  ...
  },
  {
    ...
    title:
      "🔴 **첫 장에서만 문항이 잘린다** — 칸이 79px 좁다 (원장님 확정 대기)",
    changed:
      "첫 장 문항 칸은 405px 인데 이어지는 장은 484px 다(머리글 30px + 「◆ 핵심 개념 정리」 상자 43px). " +
      "그래서 **같은 문항이 1·2번이면 잘리고 3번이면 멀쩡**하다. 2026-08-21 전수 재실측: " +
      "첫 장에 안 들어가는 문항 **3,653건(7.8%)**, 이어지는 장은 1,275건(2.7%). " +
      "제품 엔진으로 시험지를 만들어 재니 25문항 기준 **넘침 0.040건/장 · 전부 들어가는 시험지 95.8%** — " +
      "**스물다섯 장에 한 장꼴**로 한 문항이 넘친다. 원장님이 종이에서 보신 그것이다. " +
      "🔴 **아직 아무것도 안 고쳤다** — 고치면 배치가 바뀌므로 D-07 확정 대상이다. " +
      "네 안을 실제 문항으로 그린 지면이 `/dev/first-page-options` 에 있다.",
    look:
      "① `/dev/first-page-options` 를 열어 네 지면을 견주고 **어느 안으로 갈지 정해 주시면** 된다 — " +
      "일부러 첫 장 칸을 넘는 실제 문항(784·765·778px)을 올려 두었다. " +
      "② 지금 지면으로 25문항을 뽑아 **첫 장 2번 문항**이 하단 「오늘의 메모」를 뚫는지. " +
      "③ 정답지 1쪽 마지막 문항 해설이 통째로 사라지는지(「빠른 정답」 상자가 자리를 먹는다).",
    lookFromSource: false,
    evidence: [
      "docs/planning/tracks/report-print-overflow-20260821.md (전수 재실측 · 네 안 비교표)",
      "src/app/dev/first-page-options/page.tsx (실물 지면 시안)",
      "scripts/qa/measure-first-page-header.tsx (79px 을 조각으로 가른다)",
      "docs/planning/tracks/reports/fix-overflow.md §7",
    ],
    scale:
      "첫 장 칸을 어떻게 하느냐로 갈린다 — 지금 405px(안 들어감 3,653) · 개념 상자를 빼면 " +
      "448px(1,995) · 첫 장에 1문항만 두면 838px(**54건**). 장 수는 25문항이면 13장 그대로, " +
