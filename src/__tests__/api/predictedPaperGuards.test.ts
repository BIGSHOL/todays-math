/**
 * 회귀 가드 — 예측 문제지 API. 적대적 리뷰 ②(adv-배점채점.md)에서 왔다.
 *
 * 리뷰가 찾은 결함들이 main 에서 고쳐진 뒤에도 초록이어야 하는 것만 남겼다:
 *  - PATCH /scores 에 같은 orderIndex 를 반복해 보내도 DB 만점 100 이 지켜진다
 *    (수리 전 재현: 응답은 100 인데 DB 실제 만점 148).
 *  - 자기 학교 근거 0편·코호트만으로 만든 시험지는 evidenceCount 0 으로 정직하게 나간다.
 * 반쪽 시험지(청사진 미충족 201 저장) 재현은 main 이 422 로 차단하게 되어 지웠다.
 */
...
> = [];
{
  ...
  for (let i = 0; i < 15; i += 1)
    ROWS.push([n++, 4, "객관식", "중", i % 2 ? UNIT_A : UNIT_B]);
...
describe("PATCH /scores — 같은 orderIndex 를 두 번 보낸다", () => {
  it("400 으로 거부하고 DB 만점이 그대로 100 이다", async () => {
    ...
    const created = (await res.json()) as {
      data: { testId: string; questionCount: number };
    };
    ...
    const patched = await updateScores(
      jsonRequest(`http://localhost/api/tests/${testId}/scores`, "PATCH", {
        scores,
      }),
      withId(testId),
    );
    const patchBody = (await patched.json()) as {
      data?: {
        totalScore: number;
        problems: Array<{ orderIndex: number; score: number }>;
      };
      error?: unknown;
    };
    console.log(
