...
/**
 * ── 마커 없는 «다음 조건» 상자 (2026-08-18 원장님) ──────────────────────────
 * "문제에 '다음 조건'이라는 말이 있는데 조건만 따로 모아서 네모 박스 안에 넣어두면
 *  훨씬 깔끔"
 *
 * `<조건>` 마커가 아예 없고 발문이 「다음 조건을 …」이라고만 한 뒤 `∘`·`⚪` 로
 * 항목을 나열하는 문항이 있다. 전수 실측: 발문이 「다음 조건」류를 말하는 문항
 * 1,936건(4.11%) 중 이미 상자인 513건을 빼고 **마커 없이 불릿만 있는 것 61건**.
 *
 * 여기서 여는 문은 좁아야 한다 — 발문 문구만 믿고 아무 기호나 항목으로 읽으면
 * `(f ∘ g)(3)` 의 합성 기호가 불릿이 된다(실측 1ed8fb78). 수식은 가리고 본다.
 */
describe("[상자 4-C] 마커 없는 «다음 조건» 상자", () => {
  it("`다음을 만족하는 … ⚪ …⚪ …⚪ …` 을 조건 상자로 (id 1e7cfc15 — 원장님 스크린샷)", () => {
    const text =
      "다음을 만족하는 세 정수 $a,~b,~c$의 합은? ⚪ $a$는 $-\\frac{5}{7}$에 가장 가까운 정수이다. ⚪ $b$는 $\\frac{4}{3}$보다 크지 않은 양의 정수이다. ⚪ $c$는 절댓값이 $2$ 초과 $4$ 미만인 음의 정수이다.";
    const [box] = boxes(text);
    expect(box).toBeDefined();
    expect(box.label).toBe("조건");
    expect(box.items).toHaveLength(3);
    expect(box.items[0]).toContain("가장 가까운 정수이다");
    // 발문은 상자 밖에 남는다.
    expect(texts(text)[0]).toContain("세 정수");
    expect(texts(text)[0]).not.toContain("가장 가까운");
  });

  it("`○` 불릿도 같은 규칙으로 (id 032b8d6d)", () => {
    const text =
      "분수 $\\frac{9}{x}$가 다음 조건을 모두 만족할 때, $x$가 될 수 있는 모든 자연수의 합은? ○ 기약분수이다$.$ ○ $\\frac{9}{x}$는 $\\frac{1}{3}$보다 크다$.$ ○ 소수로 나타내면 유한소수가 된다$.$";
    const [box] = boxes(text);
    expect(box.label).toBe("조건");
    expect(box.items).toHaveLength(3);
  });
  ...
  it("`●` 도 불릿으로 읽는다", () => {
    const text =
      "다음 조건을 모두 만족하는 세 정수의 합은? ● 첫째 조건이다. ● 둘째 조건이다. ● 셋째 조건이다.";
    expect(boxes(text)[0].items).toHaveLength(3);
  });

  /* ── 반대쪽: 상자로 만들면 **안 되는** 것 ───────────────────────────────── */

  it("합성함수 `(f ∘ g)` 는 불릿이 아니다 (id 1ed8fb78)", () => {
    // 수식을 안 가리면 `∘` 셋이 불릿으로 보여 발문이 통째로 상자가 된다.
    const text =
      "세 함수 $f,g,h$ 가 다음 조건을 만족시킬 때, 값은? $(f$ ∘ $g)(3)=3,(g$ ∘ $f)(3)=2$ 이고 $(f$ ∘ $f)(x)=x$ 이다.";
    expect(boxes(text)).toEqual([]);
  });

  it("`□` 는 불릿이 아니다 — 빈칸 표기다 (id 11830247)", () => {
    // `□ABCD` 가 두 번 나온다고 조건 항목이 되면 안 된다.
    const text =
      "다음 조건을 만족하는 모든 $□\\mathrm{ABCD}$의 넓이의 합은? $□\\mathrm{ABCD}$는 정사각형이고 $□\\mathrm{ABCD}$의 각 변의 길이는 자연수이다.";
    expect(boxes(text)).toEqual([]);
  });

  it("발문에 「다음 조건」이 없으면 불릿이 있어도 상자로 만들지 않는다", () => {
    // 문구가 유일한 근거다. 없으면 두 손 뗀다.
    const text = "값을 구하시오. ∘ 첫째 항목이다. ∘ 둘째 항목이다.";
    expect(boxes(text)).toEqual([]);
  });

  it("불릿이 하나뿐이면 목록이 아니다", () => {
    const text = "다음 조건을 만족할 때, 값은? ∘ 하나뿐인 조건이다.";
    expect(boxes(text)).toEqual([]);
  });

  it("이미 `<조건>` 마커로 상자가 되는 문항은 그대로 둔다 — 두 번 만들지 않는다", () => {
    const text =
      "다음 <조건>을 만족하는 일차함수는? < 조건 >∘평행하다.∘점을 지난다.";
    const found = boxes(text);
    expect(found).toHaveLength(1);
    expect(found[0].items).toEqual(["∘평행하다.", "∘점을 지난다."]);
  });
});
...
/**
 * ── 나열 대상 상자 (2026-08-18 원장님) ──────────────────────────────────────
 * "이런 문제의 경우 다음 작은 수를 네모 박스 안에 넣으면 더 깔끔할텐데"
 *
 * 정본 `F:\시험지변환기` 를 봤지만 **거기에도 이 규칙은 없다** —
 * `core/content_parser.py` 의 박스 판정도 마커 기반이라 베낄 규칙이 없었다.
 * 대신 그 저장소의 박스 **모양**(5×5 병합표 · 내용 셀 양옆 스페이서 ·
 * `vertAlign="CENTER"`)이 가운데 정렬 시안의 근거다.
 *
 * 전수 실측: 발문이 「나열」류를 말하는 문항 478건 중
 *   · 이미 상자 76건
 *   · **발문 뒤**에 나열이 오는 33건 ← 상자 대상
 *   · 발문 **안**에 나열이 박힌 122건 ← 상자로 빼면 문장이 끊긴다. 건드리지 않는다.
 */
describe("[상자 4-D] 나열 대상 상자", () => {
  it("발문 뒤 수식 나열을 머리 없는 상자로 (id 01482df8 — 원장님 스크린샷)", () => {
    const text =
      "다음은 작은 수부터 차례대로 나열했을 때, 두 번째 수와 다섯 번째 수의 합은? $(2)^{3},~~\\left( -\\frac{1}{2}\\right) ^{2},~~-\\frac{9}{3},~~-\\frac{7}{2},~~(-1)^{4}$";
    const [box] = boxes(text);
    expect(box).toBeDefined();
    expect(box.headerless).toBe(true);
    expect(box.items[0]).toContain("\\frac{7}{2}");
    // 발문은 상자 밖에 남는다.
    expect(texts(text)[0]).toContain("두 번째 수와 다섯 번째 수의 합은?");
    expect(texts(text)[0]).not.toContain("frac{7}{2}");
  });

  it("수식이 여러 개로 쪼개진 나열도 잡는다 (id 07505e08)", () => {
    const text =
      "주어진 수를 작은 것부터 순서대로 나열할 때, 두 번째로 작은 수는? $\left| -\frac{3}{2}\right|$, $-\frac{2}{3}$, $\left| -2\right|$, $-\frac{2}{5}$, $+3$";
    const [box] = boxes(text);
    expect(box).toBeDefined();
    expect(box.headerless).toBe(true);
  });
  ...
  it("나열이 발문 **안**에 박혀 있으면 건드리지 않는다 (id 04f52cf6)", () => {
    // 빼내면 문장이 끊긴다 — 실측 122건이 이 모양이다.
    const text =
      "세 수 $a=2√5+√3$, $b=3√3$, $c=√5+2√3$ 의 대소관계를 바르게 나타낸 것은?";
    expect(boxes(text)).toEqual([]);
  });

  it("「나열」 문구가 없으면 뒤에 나열이 있어도 상자로 만들지 않는다", () => {
    const text = "값을 구하시오? $a$, $b$, $c$, $d$, $e$";
    expect(boxes(text)).toEqual([]);
  });

  it("항목이 셋 이하면 나열로 보지 않는다", () => {
    const text = "작은 것부터 나열한 것은? $a$, $b$, $c$";
    expect(boxes(text)).toEqual([]);
  });

  it("발문 뒤에 한글 문장이 더 있으면 나열이 아니다", () => {
    const text =
      "작은 수부터 차례대로 나열한 것은? $a$, $b$, $c$, $d$ 를 각각 구하고 그 이유를 서술하시오.";
    expect(boxes(text)).toEqual([]);
  });
});
