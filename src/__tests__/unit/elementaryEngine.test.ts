...
describe.skipIf(!hasEngine)("[초등 엔진] 그림 렌더", () => {
  ...
  // ⚠️ 축 다양성 가드(아래 시험)는 「풀 − 2 ≥ 2」일 때만 산다 — 풀이 셋이 되면 최댓값이
  //    한 가지뿐이라 검사가 **스스로 물러나고**, 그 순간 「축이 굳었다」를 아무도 못 본다.
  //    지금 수 풀이 지면 실측 때문에 넷으로 줄어 **정확히 그 경계에 섰다.** 그래서 풀 자체에
  //    하한을 둔다 — 여기가 빨개지면 「가드가 죽었다」는 뜻이니 풀 설계를 다시 봐야 한다.
  it("판매 두 자료의 값 풀은 넷 이상이다 — 축 다양성 가드가 살아 있으려면", () => {
    for (const [role, pool] of [
      ...
      expect(
        pool.length,
        `${role} 풀이 ${pool.length}개다 — 셋이면 최댓값이 한 가지뿐이라 축 다양성 가드가 잠잠해진다`,
      ).toBeGreaterThanOrEqual(4);
    }
  });

  // ⚠️ 이 두 수의 «참»은 제품이 아니라 **지면 실측**이다 — 수(가로형)는 6줄이면 눈금 글자가
  //    12.303px 로 본문(12.5px) 미달, 5줄이어야 12.562px(겹침 수리된 판 실측, 2026-08-23).
  //    가격(세로형)은 눈금이 위아래로 쌓여 엔진 상한 9줄까지 무관하다.
  //    아래 렌더 시험은 상한을 `G4_SALE_LIMITS.maxTicks` 에서 **읽는다** — 리터럴로 못 박지
  //    않으면 「풀과 상수를 같이 올리는」 편집이 전부 초록으로 지나간다(2026-08-23 재현:
  //    풀에 50 추가 + 수 상한 5→9 = 49/49 초록, 지면만 조용히 깨진다).
  //    이 수를 올리려면 겹침 수리된 판에서 **다시 재서** 근거와 함께 바꿀 것.
  it("눈금 상한은 지면 실측 그대로다 — 수 5줄 · 가격 9줄", () => {
    expect(G4_SALE_LIMITS.maxTicks).toEqual({ 수: 5, 가격: 9 });
  });
  ...
  it("두 그래프 결합의 안쪽 스펙 두 장이 실제로 그려진다", async () => {
    ...
    for (let i = 0; i < SEEDS; i += 1) {
      ...
      for (const { role, step, spec } of G4_SALE_CHART_SPECS(combo)) {
        ...
        // ⚠️ 자료마다 상한이 **다르다.** 수는 가로형이라 눈금이 옆으로 늘어서 7줄부터 글자가
        //    본문 아래로 떨어지고(실측 6줄 = 12.50px 경계), 가격은 세로형이라 9줄까지 무관하다.
        //    **엔진은 9줄까지 안 던진다** — 그래서 수를 40으로 되돌리면 그림은 멀쩡히 그려지고
        //    지면만 조용히 깨진다. 그 자리를 여기서 잡는다. 숫자는 `G4_SALE_LIMITS` 에서 읽는다.
        const cap = G4_SALE_LIMITS.maxTicks[role as "수" | "가격"];
        if (ticks.length > cap) {
          bad.push(
            `${role}: 눈금이 ${ticks.length}줄이다 — ${cap}줄까지다 (지면 실측)`,
          );
        }
