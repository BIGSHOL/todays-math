 ...
 * 물리 폭으로 받을 수 있는 범위.
 *
 * ⚠️ **이 둘은 실측이 아니다.** 원장(`figure-rect-ledger.json`)이 아직 없어
 *    실제 분포를 못 쟀다. 그래서 「이 값이 흔한가」가 아니라 **「물리적으로 가능한가」**
 *    만 본다:
 *      · 1mm 미만 — 300dpi 에서 12점(화면 4px)이다. 그림일 수 없다.
 *      · 210mm 초과 — A4 폭이다. 쪽 안의 그림이 쪽보다 넓을 수 없다.
 *
 *    좁게 잡으면 **진짜 작은 그림**(치수 표시가 붙은 5mm 짜리 기호)을 버리게 된다.
 *    분포를 재기 전에 그럴듯한 문턱을 지어내지 않는다. 못 잰 것은 못 쟀다고 둔다.
 *
