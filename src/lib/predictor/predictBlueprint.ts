...
export const DEFAULT_PARAMS: PredictorParams = {
  ...
  sameRoundBoost: 4,
  priorWeight: 2,
  // 문항 수·유형은 코호트로 **당기지 않는다**(2 → 0). 학교 고유성이 확인된 항목이라
  // 전국 평균으로 당길수록 손해다.
  //
  // ⚠️ 처음엔 0.5 로 뒀다 — "과거가 1회차뿐인 학교를 규제 없이 믿게 된다"는 이유였다.
  //    그 판단은 **오염된 데이터를 보고 내린 것**이었다. 학원이 만든 '대비' 자료
  //    130편(25문 템플릿)이 이력에 섞여 있어 학교별 문항 수가 실제보다 흔들려 보였고,
  //    그래서 축소가 필요해 보였다. 대비를 걷어내고 다시 재니 두 분할(2024·2025) 모두에서
  //    0 이 문항 수·유형을 개선하고 나빠지는 항목이 없었다(11 §14).
  stylePriorWeight: 0,
