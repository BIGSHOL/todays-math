 ...
 * 아니면 403(FORBIDDEN)" 패턴을 반(Class)·학생(Student) API가 공유한다.
 * 이후 Phase 2, T2.2(진도 API)·T5(문제/시험지 API)도 동일 패턴을 재사용할 수 있다
...
export async function requireOwnedClass(
  ...
  if (!cls) return { ok: false, response: notFoundErrorResponse("반") };
...
export async function requireOwnedStudent(
  ...
  if (!student) return { ok: false, response: notFoundErrorResponse("학생") };
