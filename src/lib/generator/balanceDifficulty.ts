 ...
 * 30개 조합 × 시드 30개. 상세는 `docs/planning/tracks/reports/review-spread.md`):
 *
 * | 규칙 | 단원 커버리지 | 단원 최다몫 | 유형 최다몫 |
 * |---|---|---|---|
 * | 단원 안 봄(예전) | 66.2% | 38.8% | 33.8% |
 * | 단원 → 유형 | 98.8% | 17.9% | 41.7% |
 * | 유형 → 단원 | 91.6% | 24.2% | 33.9% |
 * | **합산 (채택)** | **95.9%** | **20.3%** | **37.3%** |
 *
 * 우선순위를 매기면 **뒤에 놓인 쪽이 그만큼 나빠진다.** 합산은 둘 중 무엇이 먼저인지
 * 정하지 않고 「이 시험지에서 제일 덜 쓴 것」을 고른다 — 커버리지의 대부분을 얻으면서
 * 유형 편중은 덜 는다. 동률일 때만 단원을 앞세운다(그때는 잃는 것이 없다).
 *
